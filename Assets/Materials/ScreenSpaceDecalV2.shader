...
// Yuqi Wang
// Implementation & modification based on:
// https://www.github.com/ColinLeung-NiloCat/UnityURPUnlitScreenSpaceDecalShader
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(v2f i) : SV_Target
            {
                ...
                half3 norm = UnpackNormal(tex2D(_NormalMap, uv));

                col *= _Tint;// tint color
                ...
                // *** Shader Lighting & Shadow WIP ***
                // learnopengl.com/Getting-started/Coordinate-Systems
                // screen space -> camera space -> world space
                // Inverse the following matrix: Viewport Transform Matrix, Projection Matrix, View Matrix

                // half3 camSpacePosNew = mul(unity_CameraInvProjection, decalSpaceScenePos).xyz;
                // half3 objSpacePosNew = mul(UNITY_MATRIX_T_MV, camSpacePosNew).xyz;
                // half3 worldSpacePosNew = mul(unity_ObjectToWorld, objSpacePosNew).xyz;

                // Apply lighting and shadow
                // half4 shadowCoord = TransformWorldToShadowCoord(worldSpacePosNew);
                // Light light = GetMainLight(shadowCoord);
                // half3 diffuse = LightingLambert(light.color, light.direction, norm);
                // col = half4(col.rgb * diffuse * light.shadowAttenuation, col.a);
